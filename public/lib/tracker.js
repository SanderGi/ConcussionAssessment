...
const tracker = {
  ...
  initStream: function () {
    ...
    // setup video events
    tracker.video.addEventListener(
      "playing",
      function () {
    ...
    if (tracker.container !== undefined && tracker.container.ready) {
      if (tracker.enableAI && tracker.container.video != null) {
        ...
        try {
          ...
          tracker.poses = await tracker.detector.estimatePoses(
  ...
  handlePoses: function () {
    ...
    if (tracker.poses && tracker.poses.length > 0) {
      ...
      for (let pose of tracker.poses) {
        ...
        if (
          tracker.enable3D &&
          pose.keypoints3D != null &&
          pose.keypoints3D.length > 0
        ) {
  ...
  drawPath: function (fromX, fromY, toX, toY, r, g, b, score) {
    ...
    // draw connection
    tracker.drawLine(
      tracker.scaleX(fromX),
      tracker.scaleY(fromY),
      tracker.scaleX(toX),
      tracker.scaleY(toY),
      r,
      g,
  ...
  showPlaybackControls: function () {
    ...
    tracker.ctx.beginPath();
    tracker.ctx.moveTo(
      tracker.canvas.width / 2 + size / 2,
      tracker.canvas.height / 2
    );
    tracker.ctx.lineTo(
      tracker.canvas.width / 2 - size / 2,
      tracker.canvas.height / 2 + size
    );
    tracker.ctx.lineTo(
      tracker.canvas.width / 2 - size / 2,
      tracker.canvas.height / 2 - size
